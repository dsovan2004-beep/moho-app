...
const CITY_CONTENT: Record<CityName, {
  ...
  Manteca: {
    ...
    utilities: [
      { name: 'PG&E (Gas & Electric)',   desc: 'Set up your gas and electricity account online', url: 'https://www.pge.com' },
      { name: 'City of Manteca (Water)', desc: 'Water & sewer billing through the city',         url: 'https://www.mantecacity.org' },
      { name: 'Recology (Trash)',        desc: 'Curbside pickup — bins provided after signup',   url: 'https://www.recology.com' },
      { name: 'Comcast / AT&T',          desc: 'Internet & cable options in most neighborhoods', url: 'https://www.xfinity.com' },
    ],
    essentials: [
...
function Section({
  ...
  id: string; emoji: string; title: string; viewAllHref: string; children: ReactNode
}) {
  return (
...
export default async function NewResidentCityPage({ params }: PageProps) {
  ...
  // ── Parallel DB queries ──────────────────────────────────────────────────────
  let healthBizzes:    Business[] = []
  let foodBizzes:      Business[] = []
  let essentialBizzes: Business[] = []
  ...
  try {
    ...
    healthBizzes    = (healthRes.data    ?? []) as Business[]
    foodBizzes      = (foodRes.data      ?? []) as Business[]
    essentialBizzes = (essentialRes.data ?? []) as Business[]
  } catch {
    // Supabase unavailable — page renders with static content + empty biz sections
  }
